...
let sourceChanges = allSourceFiles.first(where: { $0.hasPrefix("CiWorkshop") })
...
if prTitle.count < 5 {
    warn("PR title is too short. 🙏 Please use this format `CW-000 Your feature title` and replace `000` with Jira task number.")
}
if !prTitle.contains("CW-") {
    warn("PR title does not containe the related Jira task. 🙏 Please use this format `CW-000 Your feature title` and replace `000` with Jira task number.")
}
